...
describe('settingsSchema', () => {
	describe('validateSettings', () => {
		...
		it('should reject invalid enum values', () => {
			...
			expect(result.valid).toBe(false);
			expect(result.errors.some((e) => e.includes('notificationsLevel'))).toBe(
				true,
			);
		});
		...
		it('should reject dangerous keys (prototype pollution)', () => {
			...
			const hasDangerousKeyErrors =
				result.errors.some((e) => e.toLowerCase().includes('dangerous')) ||
		...
		it('should reject settings file with too many keys', () => {
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.toLowerCase().includes('too many')),
			).toBe(true);
		});
